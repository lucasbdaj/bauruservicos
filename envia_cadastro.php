...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $servicos_endereco = filter_input(INPUT_POST, 'servicos_endereco', FILTER_SANITIZE_STRING);
    $senha = $_POST['senha'];
...
if (!preg_match("/^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*()_+|~=`{}\[\]:'\";<>?,.\/-]).{8,}$/", $senha)) {
    ...
    $stmt->bind_param("sisssssssssss", $nome_profissional, $id_profissao, $data_nascimento, $tempo_profissao, $descricao, $telefone, $email, $rede_social, $link_google, $site_prestador, $endereco, $servicos_endereco, $senha_hash);
